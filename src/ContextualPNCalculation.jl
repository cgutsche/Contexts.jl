...
function runPN(pn::CompiledPetriNet)
    ...
    f_normal::Matrix{Float64}  = zeros(Float64, 1, nTransitions)
    f_inhibitor::Matrix{Float64}  = zeros(Float64, 1, nTransitions)
    f_test::Matrix{Float64}  = zeros(Float64, 1, nTransitions)
    f_context::Matrix{Float64}  = zeros(Float64, 1, nTransitions)
    f::Matrix{Float64}  = zeros(Float64, 1, nTransitions) 
    f = zeros(Float64, 1, nTransitions)
    c = zeros(Float64, size(pn.tokenVector, 1))
...
"""
    runPNInf(pn::CompiledPetriNet)

Executes a compiled Petri net infintely.
...
Returns nothing, but modifies the Petri net state and context activations.
"""
function runPNInf(pn::CompiledPetriNet)
    # Get number of contexts dimensions of the Petri net
    nContexts::Int64 = size(pn.ContextMatrices[1])[2]
    nTransitions::Int64 = size(pn.WeightMatrix_in)[2]
